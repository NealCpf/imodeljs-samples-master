...
*--------------------------------------------------------------------------------------------*/
import { ClientRequestContext, Config, Id64, Id64String, OpenMode } from "@bentley/bentleyjs-core";
import { ContextRegistryClient, Project } from "@bentley/context-registry-client";
// make sure webfont brings in the icons and css files.
import "@bentley/icons-generic-webfont/dist/bentley-icons-generic-webfont.css";
import { IModelQuery } from "@bentley/imodelhub-client";
import { ColorDef, ElectronRpcConfiguration, FeatureAppearance, FeatureAppearanceProps, LinePixels, RgbColor } from "@bentley/imodeljs-common";
import { AuthorizedFrontendRequestContext, EmphasizeElements, FrontendRequestContext, IModelApp, IModelConnection, RemoteBriefcaseConnection, SnapshotConnection, ViewState } from "@bentley/imodeljs-frontend";
...
let emph: EmphasizeElements | undefined = undefined;

async function elementSelectedEvent(
  ...
  if (hiliteSet.elements) {
    ...
    const f = FeatureAppearance.fromJSON(props);
    emph.defaultAppearance = f;
    const c = ColorDef.from(255, 255, 0);
    emph.emphasizeElements(hiliteSet.elements, vp, undefined, true);

    await IModelApp.viewManager.selectedView!.zoomToElements(
      hiliteSet.elements
    );
  }
  ...
  private _onClickOpen = async () => {
    ...
    try {
      ...
      if (true) {
        ...
        const offlineIModel = "..//data//dian.bim";
        imodel = await SnapshotConnection.openFile(offlineIModel);
        Presentation.selection.selectionChange.addListener(elementSelectedEvent);
      } else {
